...
static const struct regmap_access_table sn65dsi83_readable_table = {
	/* 指向“允许读取的寄存器范围表”，regmap 会据此判断某地址是否可读。 */
	.yes_ranges = sn65dsi83_readable_ranges,
	/* 上面可读范围表中的项数，供 regmap 遍历和边界检查使用。 */
...
static const struct regmap_config sn65dsi83_regmap_config = {
	/* 芯片寄存器地址宽度为 8 bit，即寄存器地址占 1 字节。 */
	.reg_bits = 8,
	/* 芯片寄存器数据宽度为 8 bit，即每个寄存器值占 1 字节。 */
	.val_bits = 8,
	/* 指定哪些寄存器地址是允许读取的，regmap 会按这个表做访问检查。 */
	.rd_table = &sn65dsi83_readable_table,
	/* 指定哪些寄存器地址是允许写入的，避免误写保留或无效寄存器。 */
	.wr_table = &sn65dsi83_writeable_table,
	/* 指定易变寄存器：这类寄存器值可能被硬件实时改变，不能完全信任缓存。 */
	.volatile_table = &sn65dsi83_volatile_table,
	/* 使用 RBTREE 作为寄存器缓存结构，适合寄存器地址较稀疏的设备。 */
	.cache_type = REGCACHE_RBTREE,
	/* 芯片支持的最大寄存器地址，超过这个范围的访问会被视为非法。 */
...
//	dsi->mode_flags = MIPI_DSI_MODE_VIDEO | cc  |
//		  MIPI_DSI_MODE_LPM ;
			   ...
			   const struct i2c_device_id *id)
{
	 ...
	 * probe() 是驱动的“构造阶段”。
	 *
	 * 从 DRM 完整显示链路的角度看，probe() 此时还不会点亮屏幕，
	 * 也不会写入最终显示时序相关寄存器。它的职责是先把所有静态资源
	 * 收集好，并把这颗芯片描述给 DRM 框架，为后面的 atomic commit
	 * 真正点亮显示链路做准备。
	 *
	 * 换句话说：
	 * - probe() 负责把软件对象和资源准备好
	 * - attach() 负责把本 bridge 挂到上游 DSI host 和下游 panel/bridge 上
	 * - atomic_pre_enable()/atomic_enable() 才是在 modeset 时真正上电、
	 *   写寄存器并让链路工作
	 */
	...
	/*
	 * 为 bridge 分配私有上下文。
	 * 这里使用 devm_* 风格的资源管理，设备释放时内存会自动回收，
	 * 因此 probe 的错误处理路径会更简单。
	 */
	ctx = devm_kzalloc(dev, sizeof(*ctx), GFP_KERNEL);
	if (!ctx)
		return -ENOMEM;

	/* 保存 device 指针，后面打印日志、申请资源时都会用到。 */
	ctx->dev = dev;

	/*
	 * 确定当前驱动的是哪一种芯片型号。
	 *
	 * 如果系统通过设备树启动，就从 OF 匹配表里取出对应的 model；
	 * 如果不是设备树场景，就退回到传统的 I2C ID 表。
	 *
	 * 这一步很重要，因为 SN65DSI83 和 SN65DSI84 支持的 LVDS 拓扑
	 * 不完全相同，后面的 parse_dt() 会根据型号解释不同的 port 连接关系。
	 */
	if (dev->of_node) {
	...
	/*
	 * 申请可选的 enable GPIO，并立即把它拉低。
	 *
	 * 从 DRM 流程角度看：
	 * - probe() 先让 bridge 保持在安全的复位/关闭状态
	 * - 后面的 atomic_pre_enable() 才会在真正点屏前，按照正确时序把它拉起
	 *
	 * 之所以这样拆开，是因为 probe 可能发生在真正用到显示链路之前很久，
	 * 不能在这个阶段就把芯片过早带到半上电或工作态。
	 */
	...
	/*
	 * 满足最小复位低电平保持时间。
	 * 这样可以保证硬件先进入一个已知稳定状态，再继续做后面的解析和注册。
	 */
	usleep_range(10000, 11000);

	/*
	 * 从设备树解析静态显示拓扑：
	 * - 输入侧：上游 DSI host 的 endpoint，以及 DSI lane 数
	 * - 输出侧：下游 panel 或下一级 bridge
	 * - 板级资源：供电 regulator、双路 LVDS 接线方式等
	 *
	 * 这是理解 DRM 显示链路时最关键的步骤之一：
	 * probe() 并不是“动态发现”显示拓扑，而是把设备树 graph 中描述的
	 * 连接关系重新整理出来，并保存在 ctx 中，供后续 attach/enable 阶段使用。
	 */
	...
	/*
	 * 创建 regmap，用于后续所有芯片寄存器访问。
	 * 真正的大量寄存器编程发生在 atomic_enable() 中，但 probe() 必须先把
	 * 这个寄存器访问抽象准备好。
	 */
	...
	/* 把私有上下文挂到通用 device/I2C 对象上，方便后续反查。 */
	dev_set_drvdata(dev, ctx);
	i2c_set_clientdata(client, ctx);

	/*
	 * 把这颗芯片注册为一个 DRM bridge。
	 *
	 * 这是它进入 DRM bridge 框架的关键一步：
	 * - funcs 告诉 DRM：这个 bridge 实现了哪些回调
	 * - of_node 让 DRM/其它组件能把它和设备树节点对应起来
	 * - drm_bridge_add() 让这个 bridge 对外可见，后续上游 encoder 才能 attach 到它
	 *
	 * 这一行成功后，说明“软件世界里的 bridge 对象”已经建好了，
	 * 但它仍然是空闲的。真正运行时的顺序是：
	 *   encoder -> bridge attach -> atomic_pre_enable -> atomic_enable
	 */
...
err_put_node:
	/* parse_dt() 里可能给 host_node 增加了引用计数，失败路径要记得释放。 */
	of_node_put(ctx->host_node);
	return ret;
}
